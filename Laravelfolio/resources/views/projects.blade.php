...
<section class="section section-projet my-5">
    ...
    <div class="row">
        <div class="col-12 col-xl-6">
            <img class="rounded-3 p-0 shadow col " src="{{ asset('img/').'/'.$project->image }}" alt="{{$project->titre}}">
        ...
        <div class="col-12 col-xl-6">  
            @if (empty($project->lien_video))
                <img class="no-videos-img" src="{{ asset('img/no_vids.svg') }}" alt="">
            @else
            <iframe class="rounded-3 p-0 shadow-lg" width="100%" height="420px" src="//www.youtube.com/embed/{{$project->lien_video}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
            @endif
...
<section class="text-center section section-rss py-3 ">
    <div class="row">
        <div class="col-12 col-xl-6 rounded-4">
            <img class="rounded-3 p-0 shadow-sm" style="object-fit:cover"  src="{{$rss[$randomNumber]->enclosure->attributes()->url}}" alt="{{$rss[$randomNumber]->title}}">
        </div>
