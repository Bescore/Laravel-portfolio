...
var message = {
  ...
  handleFormSubmit: function () {
    var _handleFormSubmit = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(e) {
      ...
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          case 0:
            ...
            // creer un formData
            formData = new FormData(message.form); // on check les données
            if (!message.checkEntries(formData.get('nom'), formData.get('email'), formData.get('message'))) {
              _context.next = 14;
              break;
            }
            _context.next = 5;
            return fetch(window.location.protocol + "//" + window.location.host + "/api/message", {
              method: 'POST',
              headers: {
  ...
  checEachEntries: function checEachEntries(e) {
  ...
  changeOutlineColor: function changeOutlineColor(target, nom, regex) {
    if (target.name == nom) {
  ...
  checkEntries: function checkEntries(nom, email, message) {
    var regexEmail = /^[^<>()\[\]\\.,;:\s@"]+(?:\.[^<>()\[\]\\.,;:\s@"]+)*@(?:[^<>()\[\]\\.,;:\s@"]+\.)+[^<>()\[\]\\.,;:\s@"]+$/;
    var regexNom = /^[a-zA-ZÀ-ÿ]+(?:[-' ][a-zA-ZÀ-ÿ]+)*$/;
    var regexMessage = /^[a-zA-Z0-9\s.,:'ÀÁÂÃÄÅàáâãäåÈÉÊËèéêëÎÏîïÔÕÖÙÚÛÜùúûüÿÇçŒœÆæØøœ]+$/;
